...
// Волны: состав контролируется номером волны, спавн — по очереди из случайных ворот
export class Waves {
  constructor(enemies, arena, hooks) {
    this.enemies = enemies; this.arena = arena; this.hooks = hooks || {};
    this.num = 0; this.state = 'idle'; this.t = 0;
    this.queue = []; this.spawnT = 0; this.pending = []; this.totalThisWave = 0;
  }
  reset() {
    this.num = 0; this.queue = []; this.pending = [];
    this.state = 'intermission'; this.t = 3.5;
    this.hooks.countdown && this.hooks.countdown(3.5, 1);
  }
  startIntermission(sec) {
    this.state = 'intermission'; this.t = sec;
  }
  _composition(n) {
    const q = [];
    ...
    // перемешиваем, но громил ближе к концу
  ...
  update(dt) {
    ...
    if (this.queue.length > 0) {
      ...
      if (this.spawnT <= 0) {
        ...
        const sp = pick(this.arena.spawnPoints);
        this.arena.spawnPoints; // ворота
        const px = sp.x + rand(-1.5, 1.5), pz = sp.z + rand(-1.5, 1.5);
      ...
      // предохранитель: если враги застряли и ничего не меняется — добиваем их
      const alive = this.enemies.aliveCount;
      if (alive === this._lastAlive) this.stallT = (this.stallT || 0) + dt; else this.stallT = 0;
      this._lastAlive = alive;
      if (this.stallT > 30) { this.stallT = 0; this.enemies.killAllInstant(); }
    }
  }
}
